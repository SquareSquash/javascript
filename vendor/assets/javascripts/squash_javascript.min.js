...
k=[],h,n=0,C=m.length;n<C;n+=2)if(h=f.exec(m[n])){var v={line:+h[1],column:+h[2],func:h[3]||h[4],args:h[5]?h[5].split(","):[],url:h[6]};!v.func&&v.line&&(v.func=g(v.url,v.line));if(v.line)try{v.context=d(v.url,v.line)}catch(X){}v.context||(v.context=[m[n+1]]);k.push(v)}c=k.length?{mode:"stacktrace",name:a.name,message:a.message,stack:k}:null;if(c)return c}catch(P){if(x)throw P;}try{if(a.stack){var f=/^\s*at (?:new )?(.+) (?:\[as \S+\] )?\(((?:file|http):.*?):(\d+)(?::(\d+))?\)\s*$/i,m=/^\s*(\S*)(?:\((.*?)\))?@((?:file|http).*?):(\d+)(?::(\d+))?\s*$/i,
